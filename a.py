...
bot = telebot.TeleBot('8150960947:AAHy-pZeVBt1LpV-de0bOmdj392C4z7E9wg')
...
# Function to handle the reply when free users run the /lala command
...
# Dictionary to store the last time each user ran the /lala command
lala_cooldown = {}


# Handler for /lala command
@bot.message_handler(commands=['lala'])
def handle_lala(message):
    user_id = str(message.chat.id)
    if user_id in allowed_user_ids:
        # Check if the user is in admin_id (admins have no cooldown)
        if user_id not in admin_id:
            # Check if the user has run the command before and is still within the cooldown period
            if user_id in lala_cooldown and (datetime.datetime.now() - lala_cooldown[user_id]).seconds < 0:
                response = "❕𝗖𝗼𝗼𝗹𝗱𝗼𝘄𝗻 𝗶𝗻 𝗲𝗳𝗳𝗲𝗰𝘁 𝗽𝗹𝗲𝗮𝘀𝗲 𝘄𝗮𝗶𝘁 300 𝘀𝗲𝗰𝗼𝗻𝗱𝘀❕"
                bot.reply_to(message, response)
                return
            # Update the last time the user ran the command
            lala_cooldown[user_id] = datetime.datetime.now()
...
# Add /mylogs command to display logs recorded for lala and website commands
